 ...
 console.log(variants);
  ...
  <div className="container px-5 py-16 mx-auto">
    <div className="lg:w-4/5 mx-auto flex flex-wrap">
      <img alt="ecommerce" className="lg:w-1/2 w-full lg:h-auto h-80object-cover object-top rounded"  src={`${product.img}`}/>
      <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-10">
        ...
        <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
          <div className="flex">
           ...
           {Object.keys(variants).includes('blue') && Object.keys(variants['red']).includes(size) && <button className="border-2 border-gray-300 ml-1 bg-blue-700 rounded-full w-6 h-6 focus:outline-none"></button>}
           ...
           {Object.keys(variants).includes('red') && Object.keys(variants['blue']).includes(size) && <button className="border-2 border-gray-300 ml-1 bg-red-500 rounded-full w-6 h-6 focus:outline-none"></button>}
        ...
        <div className="flex">
          ...
          <button className="flex ml-7 text-white bg-pink-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded">Buy Now</button>
          <button onClick={()=>{{addToCart('wear-the-code',1,499,'Wear the code(XL Red)','XL','Red')}}} className="flex ml-7 text-white bg-pink-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded">Add to Cart</button>
        ...
        <div className="pin flex mt-6 space-x-2 text-sm">
           <input onChange={onChangePin} type="text" placeholder="Enter your pincode" className='px-2 border-2 border-gray-400 rounded-md' /> 
           <button onClick={checkservicablity}className=' text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded'>Check</button>
        </div> 
...
//  console.log(context);
// title can be similar but slug cannot 
let product = await Product.findOne({slug:context.query.slug}) ;   
console.log(product);
let variants = await Product.find({title:product.title}) ;  // This is an array of variants  
let colorSizeSlug = {};   // {color:{size:{slug:'wear-the-code}}}
